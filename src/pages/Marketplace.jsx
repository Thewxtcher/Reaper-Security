...
import {
  Store, Palette, ArrowRight, Package, Plus, Code2, Upload, Star,
  Download, Tag, Check, X, Eye, Globe, Lock, Search, Filter, Zap
} from 'lucide-react';
...
// Version: 1.0.0
//
// This is a site plugin for ReaperSecurity.
// You can inject UI elements, add functionality, etc.
// 
// Available globals:
//   window.reaper - platform utilities
//   document - DOM access
//
...
(function() {
  ...
  console.log('[Plugin] My Awesome Plugin loaded!');
})();
`;

function PluginCard({ plugin, user, onEnable, onDisable, onVote }) {
  const isEnabled = plugin.enabled_by?.includes(user?.email);
  const hasVoted = false; // simplified
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {user && (
                <Button
                  size="sm"
                  onClick={() => isEnabled ? onDisable(plugin) : onEnable(plugin)}
...
function CreatePluginDialog({ open, onClose, user, onCreated }) {
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Input value={form.tags} onChange={e => setForm({...form, tags: e.target.value})}
                placeholder="ui, dark, custom" className="bg-[#0a0a0a] border-white/10 text-white mt-1" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-gray-400 text-sm cursor-pointer">
              ...
              Make this plugin public (visible in marketplace)
            </label>
          </div>
    ...
    <div className="min-h-screen py-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4 mb-6 text-sm text-gray-500">
          <span><span className="text-white font-medium">{filtered.length}</span> plugins</span>
          <span>•</span>
          <span><span className="text-white font-medium">{plugins.filter(p => p.enabled_by?.includes(user?.email)).length}</span> enabled by you</span>
        </div>
